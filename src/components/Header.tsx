...
const getSocialIcon = (label: string) => {
  ...
  if (lower.includes("github"))
    return (
      <Github className="h-4 w-4 shrink-0 text-zinc-500 transition-colors duration-200 group-hover/item:text-black group-hover/subitem:text-black" />
    );
  if (lower.includes("linkedin"))
    return (
      <Linkedin className="h-4 w-4 shrink-0 text-zinc-500 transition-colors duration-200 group-hover/item:text-black group-hover/subitem:text-black" />
    );
  if (lower.includes("email") || lower.includes("mail"))
    return (
      <Mail className="h-4 w-4 shrink-0 text-zinc-500 transition-colors duration-200 group-hover/item:text-black group-hover/subitem:text-black" />
    );
  return (
    <ExternalLink className="h-4 w-4 shrink-0 text-zinc-500 transition-colors duration-200 group-hover/item:text-black group-hover/subitem:text-black" />
  );
};
...
export default function Header({
  ...
  const { transitionTarget, triggerTransition } = useTransition();
          ...
          <div ref={logoRef}>
            ...
            >
              ...
              <div className="3xl:text-3xl 4xl:text-4xl 5xl:text-5xl relative h-[1.7em] w-fit overflow-hidden text-xl leading-[0.85] font-extrabold tracking-normal uppercase select-none sm:text-2xl">
                <div
                  className={`flex flex-col transition-transform duration-300 ease-in-out ${transitionTarget !== null ? "-translate-y-1/2" : "translate-y-0"}`}
                >
                        ...
                        >
                          ...
                          >
                            {item.dropdownItems.map((subItem) => {
                              ...
                              const SocialIcon = getSocialIcon(subItem.label);
                              return (
                                <a
                                  key={subItem.label}
                                  href={subItem.href}
                                  target={
                                    isExternal && !subItem.href.startsWith("mailto")
                                      ? "_blank"
                                      : undefined
                                  }
                                  rel={
                                ...
                                >
                                  ...
                                  {isExternal && (
                                    <ArrowRight className="3xl:h-4 3xl:w-4 4xl:h-4.5 4xl:w-4.5 5xl:h-5 5xl:w-5 ml-auto h-3.5 w-3.5 -translate-x-1 text-black opacity-0 transition-all duration-200 group-hover/item:translate-x-0 group-hover/item:opacity-100" />
                                  )}
                                </a>
                              );
                            })}
                  ...
                  {item.dropdownItems ? (
                    <>
                      ...
                      >
                        {item.dropdownItems.map((subItem) => {
                          const isExternal =
                            subItem.href.startsWith("http") || subItem.href.startsWith("mailto");
                          const SocialIcon = getSocialIcon(subItem.label);
                          return (
                            <a
                              key={subItem.label}
                            ...
                            >
                              ...
                              {isExternal && (
                                <ArrowRight className="mr-3 ml-auto h-4 w-4 -translate-x-1 text-black opacity-0 transition-all duration-200 group-hover/subitem:translate-x-0 group-hover/subitem:opacity-100" />
                              )}
                            </a>
                          );
                        })}
